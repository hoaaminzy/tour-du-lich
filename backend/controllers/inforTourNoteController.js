const inforTourNoteModel = require("../models/inforTourNoteModel");
const asyncHandle = require("express-async-handler");

// create user
const addInforTourNote = async (req, res) => {
  // const { _id } = req.user;
  try {
    const newInforTourNote = await inforTourNoteModel.create({
      ...req.body,
      // updateBy: _id,
    });

    res.status(201).send({
      newInforTourNote: newInforTourNote,
      success: true,
...
const getAllInforToursNote = async (req, res) => {
  try {
    const inforToursNote = await inforTourNoteModel.find({});
    res.status(201).send({
      inforToursNote,
      success: true,
